function [Standards, WB_list] = CWPdet_eScience(Data, stride_list, n_min, n_min_Strides, max_break,  rMrkL, lMrkL, headerTraj, fs, ...
    TurnM, TurnDur, TurnsNotDec, Euler_Angles, maxTurn, averVel,  maxTurn_WB, averVel_WB, PelvicMrk, otherAngleFoot, thrs)
    Standards = [];
    WB_list = [];
    ...
    if ~isempty(stride_list.R)
        rICS_now = [stride_list.R.ICEvents]; rICS_now = rICS_now(2:2:end); rICS_adj = rICS_now(~[stride_list.R.LastStrideFlag]);
    else
        rICS_adj = [];
%         stride_list.R.ICEvents = [nan, nan];
%         stride_list.R.TrueStrideFlag = false;
    end
    if ~isempty(stride_list.L)
        lICS_now = [stride_list.L.ICEvents]; lICS_now = lICS_now(2:2:end); lICS_adj = lICS_now(~[stride_list.L.LastStrideFlag]);
    else
        lICS_adj = [];
%         stride_list.L.ICEvents = [nan, nan];
%         stride_list.L.TrueStrideFlag = false;        
    end
    
    %% Adjust Turn Magnitude and duration
    if ~isempty(stride_list.L) && ~isempty(stride_list.R)
        begMov = min([min([stride_list.R.ICEvents]),min([stride_list.L.ICEvents])]);
    elseif ~isempty(stride_list.L)
        begMov = min([stride_list.L.ICEvents]);
    else
        begMov = min([stride_list.R.ICEvents]);
    end
    endMov = max([max(rICS_adj),max(lICS_adj)]);    
    
    if ~isempty(TurnDur)
        posToChange = [];
        if TurnDur(1,1)< begMov
            TurnToChange = TurnDur(TurnDur(:,1)< begMov | TurnDur(:,2)< begMov,:);        
            for k = 1:size(TurnDur,1)
                for j = 1:size(TurnToChange,1)
                    if (TurnToChange(j,1) == TurnDur(k,1) && TurnToChange(j,2) == TurnDur(k,2))
                        posToChange = [posToChange; k];
                    end
                end
            end
        else
            if ~isempty(find(TurnDur(:,1)< begMov,1))
                tt = 2;
                while TurnDur(tt,1)< begMov && tt<size(TurnDur,1)-1
                    tt = tt+1;
                end
                TurnToChange = TurnDur(TurnDur(:,1)< begMov | TurnDur(:,2)< begMov,:);        
                for k = 1:size(TurnDur,1)
        ...
        if TurnDur(end,2)> endMov
            TurnToChange = TurnDur(TurnDur(:,1)> endMov | TurnDur(:,2)> endMov,:);        
            for k = 1:size(TurnDur,1)
                for j = 1:size(TurnToChange,1)
                    if (TurnToChange(j,1) == TurnDur(k,1) && TurnToChange(j,2) == TurnDur(k,2))
                        posToChange = [posToChange; k];
            ...
            if ~isempty(find(TurnDur(:,2)> endMov,1))
                tt = size(TurnDur,1)-1;
                while TurnDur(tt,2)> endMov && tt>1
                    tt = tt-1;
                end
                TurnToChange = TurnDur(TurnDur(:,1)> endMov | TurnDur(:,2)> endMov,:);        
                for k = 1:size(TurnDur,1)
        ...
        if ~isempty(posToChange)
            ...
            for k = 1:size(posToChange,1)
                checkNow = TurnDur(posToChange(k),:);
                if ~isempty(find(checkNow<begMov, 1))
                    if TurnDur(posToChange(k),2)<begMov
                        % this turn can be removed
                        TurnDur_temp(posToChange(k),:) = [0 0];
                        TurnM_temp(posToChange(k)) = 0;
                    else
                        TurnDur_temp(posToChange(k),1) = begMov;
                        TurnM_temp(posToChange(k)) = Euler_Angles(TurnDur_temp(posToChange(k),2)) - Euler_Angles(TurnDur_temp(posToChange(k),1));
                    end
                end

                if ~isempty(find(checkNow>endMov, 1))
                    if TurnDur(posToChange(k),1)>endMov
                        % this turn can be removed
                        TurnDur_temp(posToChange(k),:) = [0 0];
                        TurnM_temp(posToChange(k)) = 0;
                    else
                        TurnDur_temp(posToChange(k),2) = endMov;
                        TurnM_temp(posToChange(k)) = Euler_Angles(TurnDur_temp(posToChange(k),2)) - Euler_Angles(TurnDur_temp(posToChange(k),1));
                    end
                end
            end
%             TurnM = TurnM_temp(abs(TurnM_temp)>0); TurnDur = TurnDur_temp(TurnDur_temp(:,1)>0,:);
            TurnM = TurnM_temp(TurnDur_temp(:,1)>0); TurnDur = TurnDur_temp(TurnDur_temp(:,1)>0,:);
            clear ('TurnM_temp', 'TurnDur_temp');
        end
    end
    ...
    CandidateWBs = reDefineWBs(WB_L, WB_R);
     ...
     %% CWPs detected on only one side must have at least 3 correct strides
     if ~isempty(CandidateWBs)
...
%                 if allStrides(k-1,6) == 1 && allStrides(k,6) == 1
        ...
        elseif ~isempty(r)
            ...
            elseif strcmp(TerminationReason_L(l).WB,'Pause')
                ...
                %% CHECK IF THISHAS TO BE KEPT FOR CWP
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                [allStrides, WB_e, StrNowR, StrNowL] = removeLastStride(allStrides, StrNowR, StrNowL, stride_list);
                %%
            end            
        ...
        if size(allStrides,1) >= n_min_Strides          
            ...
            if caseWB == 1
                ...
                OverallDistance_COM_cum = [];
                allHS_cum = sort([HS_r; HS_l]);
                OverallDistance_COM_cum(1) = 0;
                for jj = 2:size(allHS_cum, 1)
                    OverallDistance_COM_cum(jj) = OverallDistance_COM_cum(jj-1) + sqrt((COM(allHS_cum(jj),1) - COM(allHS_cum(jj-1),1))^2+...
                        (COM(allHS_cum(jj),2) - COM(allHS_cum(jj-1),2))^2);
                end
            ...
            if find(isnan(HS),1) % there are NaNs
                ...
                for z = 1:size(pos_zeros,1)
                    if pos_zeros(z) == size(pos_zeros,1)
                        
                    elseif HS_temp1(pos_zeros(z),1)>HS_temp1(pos_zeros(z)-1,1) && HS_temp1(pos_zeros(z),1)<HS_temp1(pos_zeros(z),1) % HS_temp1(pos_zeros(z),1)<HS_temp1(pos_zeros(z)+1,1)
                    %OK
                    else
            ...
            if ~isempty(WB_list.(strcat('WB',num2str(i))).ALLTurn_WB)
                ...
                if maxTurn_WB>300
                    ...
                    Standards(i).Turning_SharpTurn_Flag = zeros(size(Standards(i).Turn_Start));
                    posTurn = abs(Standards(i).Turn_Angle)> maxTurn_WB & abs(Standards(i).Turn_MeanAngularVelocity)>averVel_WB;
                    Standards(i).Turning_SharpTurn_Flag(posTurn) = 1;
                end
            else
            ...
            fprintf('Strides have been removed either for gait termination conditions or during sharp turns - not enough strides for satisfying the CWP conditions!\n');
        end

        %% Remove CWPs where there are less than 3 strides when only R or L events have been identifies
        if (caseWB == 2 || caseWB == 3) && size(Standards,2)==i
            ...
            %         Standards(i) = [];
            %         WB_list = [];
            fprintf('Not enough strides for satisfying the CWP conditions!\n');
        end
    end
    if isempty(CandidateWBs)
        WB_list = [];
    ...
    if isempty(Standards)
        WB_list = [];
        fprintf('CWPs not detected, CWP conditions not satisfied!\n');
    end
...
% calculation possible(1)/notPossible(0); 3rd column: HS belongs to a
...
function [ST_d, SW_d, SS_d, DS_d, ST_l, SW_l, ST_v, SW_v] = calculateStrideStanceSwing(Strides, headerTraj, fs)
    ...
    for k = 1:size(Strides,1)
        if Strides(k,5)>0  && Strides(k,end) == 1%% FC has been identified stance/swing phase can be identified
            ...
            mrk_now1 = strcat(side,'REF'); mrkTraj = (headerTraj.(mrk_now1));
