function [IC, FC, missingIC] = GE_checkEvents_v4(Data, headerTraj, time, FootMrk, side, fs, GEs, GEs_v, max_break, foreFootICs, fig)
%%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% new version - info about GAPS is also included
% New version that takes into account ICs with the forefoot!
%%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
%% check if an IC found with the vert algo has been found near a gap
if ~isempty(IC(IC(:,end)==0,:))
    if ~isempty(StartStopGaps)
        possibleICs_to_rem = IC(IC(:,end)==0,1);
        IC(:,end+1) = ones(size(IC,1),1);        
        w = 25;
        check_event = zeros(size(possibleICs_to_rem,1),size(StartStopGaps,1));
        Check_posToRem = false(size(possibleICs_to_rem,1),1);
        for ev = 1:size(possibleICs_to_rem,1)
            % before gap
            for k = 1:size(StartStopGaps,1)
                if StartStopGaps(k,1)-w >0
                    if ~isempty(intersect(StartStopGaps(k,1)-w:StartStopGaps(k,1),possibleICs_to_rem(ev,1)))
                        check_event(ev,k)=1;
                    end
                elseif StartStopGaps(k,1)>1
                    if ~isempty(intersect(1:StartStopGaps(k,1),possibleICs_to_rem(ev,1)))
                        check_event(ev,k)=1;
                    end
                end
            end
            ...
            % after gap
            for k = 1:size(StartStopGaps,1)
                if StartStopGaps(k,2)+w <N
                    if ~isempty(intersect(StartStopGaps(k,2):StartStopGaps(k,2)+w,possibleICs_to_rem(ev,1)))
                        check_event(ev,k)=1;
                    end
                elseif StartStopGaps(k,2)>N
                    if ~isempty(intersect(StartStopGaps(k,2):N,possibleICs_to_rem(ev,1)))
                        check_event(ev,k)=1;
                    end
                end
            end
            Check_posToRem(ev) = ~isempty(find(check_event(ev,:), 1));
            if Check_posToRem(ev)
                % remove relevant ICs
                posToRem = find(IC(:,1)==possibleICs_to_rem(ev));
                if ~isempty(posToRem)
                    IC(posToRem,end)=0;
                end
            end
        ...
        IC = IC(IC(:,end)==1,:); IC = IC(:,1:2);              
    end % there are gaps in the heel mrk
end % there are events found with the vertical mrk traj

%% Check for last GEs
posToCheck = IC(IC(:,1) > nStr(end),1);
h_posToCheck = H(posToCheck);
if ~isempty(find(h_posToCheck > 0.4*max(H), 1))
    posToRem = find(h_posToCheck > 0.4*max(H));
    posToRem
    posToCheck(posToRem)
    IC(:,1)

    % Use ismember to find rows in IC(:,1) that match any of the selected values
    posToRem_n = find(ismember(IC(:,1), posToCheck(posToRem)));
