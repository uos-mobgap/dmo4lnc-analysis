function [data, Stereo, InclinationFootGait, InclinationFootStatic] = extractDMOsFromData(data,TM, tm, Test,testNow, strideParam, WB_Param,...
...
for tl = 1:size(Trials,1)
    ...
    if ~isempty(pos)
        ...
        if FlagCheck %|| ~FlagCheck
            ...
            mrkDyn = markers(contains(markers, 'BACK'));
            ...
            FootMrk = {'HEEL','TOE'};
            if ~originalZeni
                [GEs.HS.right, GEs.TO.right, Rl, rMrkL, foreFootICs.right, checkEvents.right] = GE_Zeni_v4(Data, headerTraj, time, FootMrk, 'R', fs, mrkDyn, fig,TurnsNotDec, MrkGaps, InclinationFootStatic, InclinationFootGait);
                [GEs.HS.left, GEs.TO.left, ~, lMrkL, foreFootICs.left, checkEvents.left] = GE_Zeni_v4(Data, headerTraj, time, FootMrk, 'L', fs, mrkDyn, fig, TurnsNotDec, MrkGaps, InclinationFootStatic, InclinationFootGait);
            else
            ...
            if checkEvents.right && checkEvents.left
                %% Remove the GEs found in the sections where these should not be extracted with the Zeni's algo
                Event = fieldnames(GEs);
                Side = fieldnames(GEs.HS);
                % add safety frames
                TurnsNotDecSaf = TurnsNotDec;
                if ~isempty(TurnsNotDec)
                    TurnsNotDecSaf(:,1) = TurnsNotDec(:,1)-3;
                    TurnsNotDecSaf(:,2) = TurnsNotDec(:,2)+3;
                end
                %
                for i = 1:length(Event)
                    for j = 1:length(Side)
                        if ~isempty(GEs.(Event{i}).(Side{j}))
                            if ~isempty(TurnsNotDecSaf)
                                for k = 1:size(TurnsNotDecSaf,1)
                                    [~,d] = intersect(GEs.(Event{i}).(Side{j})(:,1),TurnsNotDecSaf(k,1):1:TurnsNotDecSaf(k,2));
                                    if ~isempty(d)
                                        GEs.(Event{i}).(Side{j})(d,:)=[];
                                        if strcmp(Event{i},'HS')
                                            foreFootICs.(Side{j})(d,:)=[];
                                        end
                                    end
                                end
                            end
                ...
                %% GEs found using mrk vertical traj
                [GEs_v.HS.right, GEs_v.TO.right] = GE_vertTraj_v1(Data, headerTraj, time, FootMrk, 'R', fs, figV);
                [GEs_v.HS.left, GEs_v.TO.left] = GE_vertTraj_v1(Data, headerTraj, time, FootMrk, 'L', fs, figV);
                
                %% Add the GEs found in the other parts of the signal
                [GEs, foreFootICs] = mergeZeniandV(GEs, GEs_v, Event, Side, TurnsNotDec, foreFootICs, fs);
        ...
        if Pers ~=1
            ...
            if isstruct(CheckOutputs.HeelMrkFlag)
                ...
                %                 for k = size(elementToRem,1):-1:1
                %                     removeRow = elementToRem(k);
                %                     Stereo.ContinuousWalkingPeriod(removeRow) = [];
                %                     Stereo.MicroWB(removeRow) = [];
                %                 end
