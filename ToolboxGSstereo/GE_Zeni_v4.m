function [IC, FC, R_align, MrkL, foreFoot, checkOutputs] = GE_Zeni_v4(Data, headerTraj,time, FootMrk, side, fs, mrkDyn, fig, TurnsNotDec, MrkGaps, InclinationFootStatic, InclinationFootGait)
...
if isempty(TurnsNotDec) && length(MrkGaps) == 1 && ~contains(MrkGaps,'REF') && ~contains(MrkGaps,'WRIST') && contains(MrkGaps,'BACK')
    check = false;
else
    check = true;
end
...
if check
    for i = 1:length(Data.(headerTraj).BACK0)
        zML(i,:) = (Data.(headerTraj).BACKX(i,:) - Data.(headerTraj).BACK0(i,:))/...
            norm(Data.(headerTraj).BACKX(i,:) - Data.(headerTraj).BACK0(i,:));          % Medio-lateral axis
        v(i,:) = (Data.(headerTraj).BACKY(i,:) - Data.(headerTraj).BACK0(i,:))/...
            norm(Data.(headerTraj).BACKY(i,:) - Data.(headerTraj).BACK0(i,:));
    ...
    t = Data.(headerTraj).BACK0;                                                        % Origin of the LRF
    
else
    if strcmp(MrkGaps, 'BACKY')
        for i = 1:length(Data.(headerTraj).BACK0)
            zML(i,:) = (Data.(headerTraj).BACKX(i,:) - Data.(headerTraj).BACK0(i,:))/...
                norm(Data.(headerTraj).BACKX(i,:) - Data.(headerTraj).BACK0(i,:));          % Medio-lateral axis
            v(i,:) = (Data.(headerTraj).BACKREF(i,:) - Data.(headerTraj).BACK0(i,:))/...
                norm(Data.(headerTraj).BACKREF(i,:) - Data.(headerTraj).BACK0(i,:));
            xAP(i,:) = cross(v(i,:),zML(i,:))/norm(cross(v(i,:),zML(i,:)));                 % Anterior-Posterior axis
...
for i = 1:length(zML)
    ...
    REF_l(i,:) = (R_align(:,:,i)'*(Data.(headerTraj).(strcat(side,'REF'))(i,:) - t(i,:))')';
    %INDIP_l(i,:) = (R_align(:,:,i)'*(Data.(headerTraj).(strcat(side,'_INDIP'))(i,:) - t(i,:))')';
end
MrkL.HEEL = HEEL_l;
MrkL.TOE = TOE_l;
MrkL.COM = t;
